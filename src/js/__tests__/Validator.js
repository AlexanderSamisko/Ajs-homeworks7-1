...
test('testing whether method able to validate proper nickname (right case)', () => {
  const str = 'The-Iron_man999kicks';
  const logSpy = jest.spyOn(console, 'log');
  Validator.validateUsername(str);
  expect(logSpy).toHaveBeenCalledWith('Nickname is acceptable!');
});

test('testing whether method able to validate proper nickname (wrong case - some letters besides latin)', () => {
  const str = 'The-Iron_мужик999kicks';
  expect(() => {
    Validator.validateUsername(str);
